...
async def delete_token(
    api_token: Annotated[ApiTokenStr, Path()],
    session_maker: DBSession,
    token_data: TokenData,
) -> ApiTokenDetailsResponse:
    async with session_maker() as session:
        deleted_token = await delete_token_script(session, token=api_token, customer_id=None if token_data.is_(UserRole.ADMIN) else token_data.id)
