          ...
          <ItemActions v-if="data.user.id === token?.id">
						...
						<Dialog v-model:open="isDialogOpen">
							...
							<DialogContent className="sm:max-w-sm">
								...
								<UForm :schema="editEmailSchema" :state="editEmailState" class="space-y-4" @submit="editEmail">
									<UFormField label="New email" name="new_email" class="w-full">
										<UInput v-model="editEmailState.new_email" class="w-full" placeholder="your.email@mail.com"/>
									</UFormField>

									<DialogFooter>
										<UButton label="Send code" type="submit" :loading="isRequestingEmailReset"/>
									</DialogFooter>
								</UForm>
									
								<UForm :schema="validateEmailSchema" :state="validateEmailState" class="space-y-4" @submit="validateEmail">
									<UFormField label="Validation code" name="code">
										<UInput v-model="validateEmailState.code" class="w-full" placeholder="******"/>
									</UFormField>

									<DialogFooter>
										<DialogClose asChild>
											<UButton variant="outline" label="Cancel" />
										</DialogClose>
										<UButton type="submit" label="Save changes" :loading="isValidatingEmailReset" />
									</DialogFooter>
								</UForm>
							</DialogContent>
						</Dialog>
